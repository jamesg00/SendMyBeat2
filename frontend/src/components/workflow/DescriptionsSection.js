...
const DescriptionsSection = ({
  ...
  const renderSavedCard = () => (
    <Card className={`dashboard-card h-full workflow-output-card ${hasSavedDescriptions ? "" : "workflow-output-card--empty"}`}>
      <CardHeader className="px-4 sm:px-6 py-4 sm:py-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            <CardTitle className="text-sm sm:text-base">Saved Descriptions</CardTitle>
            <CardDescription>Reusable versions live here once you save them.</CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <Button
              size="sm"
  ...
  if (mode === "results") return renderSavedCard();
